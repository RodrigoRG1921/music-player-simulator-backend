...
const createSongsFile = () => {
  try {
    const file = fs.createWriteStream(`${process.env.DEFAULT_FILE_PATH}/${process.env.SONGS_FILE_NAME}`)
...
const loadSongsFromFile = async ({
  ...
  try {
    const pathFile = path || `${process.env.DEFAULT_FILE_PATH}/${process.env.SONGS_FILE_NAME}`
...
const addSong = (newSong) => {
  try {
    const pathFile = `${process.env.DEFAULT_FILE_PATH}/${process.env.SONGS_FILE_NAME}`
